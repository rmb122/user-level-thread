...
void ult_schedule_sleep_thread()
{
    ...
    long long int time_diff = ((long long int) curr_time.tv_sec * 1000000000 + (long long int) curr_time.tv_nsec) -
                              ((long long int) ult_last_sleep_schedule.tv_sec * 1000000000 +
                               (long long int) ult_last_sleep_schedule.tv_nsec);
...
{
    while (tcb_list->next != NULL && tcb_list->next->vpriority >= tcb->vpriority)
    { // 在一个优先度比自己低的线程前面插入, 同时在这个优先度里面是最后一个
